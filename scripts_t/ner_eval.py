...
def compare_accuracy(gold, pred, dt=None):
    ...
    for gold_sent, pred_sent in zip(gold, pred):
        ...
        if not dt:
            ...
            correct += sum([1 if ('O' not in (g, p) and g.split('-')[1] == p.split('-')[1] == dt)
                            else 0 for g, p in zip(gtags, ptags)])
            total += sum([1 if (g is not 'O' and g.split('-')[1] == dt) else 0 for g in gtags])
...
def main_func(gold_file, pred_file):
    raw_gold = [tagged for tagged in read_data(gold_file)]
    raw_pred = [tagged for tagged in read_data(pred_file)]
    gold_data = [normalize_bio(tagged) for tagged in raw_gold]
    pred_data = [normalize_bio(tagged) for tagged in raw_pred]
