...
class HmmModel:
    ...
    def computeFromFile (self, filePath):
        ...
        self._unknownCounter = dict(self._wordTags.computeUnknown(self.unkThreshold))

    def reComputeUnknown (self, newThreshold = 5):
        if newThreshold != self.unkThreshold:
            self._unknownCounter = dict(self._wordTags.computeUnknown(newThreshold))
    ...
    def loadTransitions (self, QfilePath = None, EfilePath = None):
        ...
        total = sum(map(lambda x: x[1], self._tagsTransitions.getItems())) - \
                self._tagsTransitions.getValue((self.startTag,))
    ...
    def getAllTags (self):
        return list(filter(lambda x: x != self.startTag, self._tagsTransitions.keys()))
    ...
    def getUnknownTag (self, tag):
        return self._unknownCounter.setdefault(tag, 0)
